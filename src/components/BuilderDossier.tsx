...
export function BuilderDossier({ permit, builder }: BuilderDossierProps) {
  ...
  const websiteUrl = builder.website
    ? (builder.website.startsWith('http') ? builder.website : `https://${builder.website}`)
    : '#';
    ...
    <div className="rounded-xl border border-slate-700/60 bg-slate-900/90 p-5 text-white shadow-xl">
      ...
      <div className="mt-5 grid grid-cols-2 gap-3">
        ...
        {builder.primary_phone ? (
          ...
          <button disabled className="rounded-lg bg-slate-800 px-3 py-2.5 text-xs text-slate-500 cursor-not-allowed">
            No Phone Listed
          </button>
        )}

        {/* Email Estimating Button (FIXED) */}
        {builder.email ? (
          <a
            href={mailtoUrl}
            target="_blank"
